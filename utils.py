...
def tolerance_parameter_customized_distribution(x):
    ...
    coscale = settings.runargs

    return np.log10((np.random.rand(x)* (max(obspec[2,:]**2)*(0.1 - 0.01))) + (0.01*min(obspec[2,10::3]**2))) 
    ...
    def refinement_params_dic_gen(self):
        ...
        for i in range(ndata):
            dictionary['params']["tolerance_parameter_%d" % (i+1)] = {
                'initialization': None,
                'distribution': ['customized', tolerance_parameter_customized_distribution],
                'range':None,
                'prior': None
            }
        return dictionary
...
def MC_P0_gen(updated_dic,model_config_instance):
