...
def lnprior(theta,re_params):
    ...
    intemp_keys = re_params.dictionary['pt']['params'].keys()
    intemp = [getattr(params_instance, key) for key in intemp_keys]
    ...
    if (proftype == 1):
        ...
        gas_profile = np.empty((count_N, press.size))
        gas_profile_index = 0   

        for i in range(len(gastype_values)):
            if  gastype_values[i]=="N":
                P_gas= getattr(params_instance, "p_ref_%s"%gas_keys[i])
                gas_alpha= getattr(params_instance, "p_ref_%s"%gas_keys[i])
                t_gas= getattr(params_instance, "p_ref_%s"%gas_keys[i])
        ...
        gas_profile = np.zeros((count_N, press.size))
        gas_profile_index = 0   
...
def priormap_dic(theta,re_params):
    ...
    if (proftype == 2):
       ...
       # Tint - prior following Molliere+2020
        ...
        # T2
        phi[T2_index] = 10. + (theta[T1_index] * 4000)
        # T3
        phi[T3_index] = 10.+ (theta[T1_index] * 4000)
...
def lnlike(theta,re_params):
    ...
    gnostics = 0
    shiftspec, photspec,tauspec,cfunc = modelspec(theta,re_params,settings.runargs,gnostics)
...
def modelspec(theta,re_params,args,gnostics):
    ...
    gases_myP,chemeq,dist,cloudtype,do_clouds,gasnum,cloudnum,inlinetemps,coarsePress,press,inwavenum,\
    linelist,cia,ciatemps,use_disort,fwhm,obspec,proftype,do_fudge,prof,do_bff,bff_raw,ceTgrid,metscale,\
    coscale = args
